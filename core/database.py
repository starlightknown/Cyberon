...
class Database:
    ...
    def add_reaction_role(self, bot_dict: dict):
        try:
            conn = sqlite3.connect(self.database)
            cursor = conn.cursor()
            if self.exists(bot_dict["message"]["message_id"]):
            ...
            while True:
                try:
                    ...
                    cursor.execute(
                        ...
                        (
                            bot_dict["message"]["message_id"],
                            bot_dict["message"]["channel_id"],
                            reactionrole_id,
                            bot_dict["message"]["guild_id"],
                            bot_dict["limit_to_one"],
                        ),
                    )
                    break
            ...
            combos = [
                ...
                for reaction, role_id in bot_dict["reactions"].items()
            ]
            cursor.executemany(
